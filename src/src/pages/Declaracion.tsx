...
export default function Declaracion() {
  ...
  const guardar = async () => {
    ...
    const { error } = await supabase.from("calculos_fiscales").insert({
      ...
      ingresos: resumen.ingresosTotal,
      gastos: resumen.gastosTotal,
      isr_estimado: resumen.isr,
      iva_estimado: Math.max(resumen.ivaAPagar, 0),
    });
    setGuardando(false);
    if (error) {
      toast.error("No pudimos guardar tu cálculo. Inténtalo nuevamente.");
      return;
    }
    void registrarAuditoria("CALCULO_GUARDADO", { periodo: `${anio}-${mes}`, isr: resumen.isr });
