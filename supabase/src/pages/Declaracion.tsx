    ...
    return (
      <div className="px-4 pt-6 space-y-6 animate-slide-up">
        ...
        <div className="rounded-2xl bg-card border border-border p-5 space-y-4">
          <p className="font-bold text-lg">¿Qué significa esto? 🤔</p>
          <ExplainRow
            emoji="💰"
            title="Tu ISR"
            text={`De cada peso que vendiste, el SAT te pide ${(r.tasaISR * 100).toFixed(2)} centavos. Por eso pagas ${formatMXN(r.isr)} de ISR este mes.`}
          />
          <ExplainRow
            emoji="🧾"
            title={r.ivaAPagar < 0 ? "Tienes IVA a favor" : "Tu IVA"}
            text={
              r.ivaAPagar < 0
                ? `Gastaste con factura más IVA del que cobraste. El SAT te debe ${formatMXN(Math.abs(r.ivaAPagar))}.`
                : `Cobraste ${formatMXN(r.ivaCobrado)} de IVA y puedes restar ${formatMXN(r.ivaAcreditable)} de tus gastos con factura. Te queda ${formatMXN(r.ivaAPagar)} por pagar.`
            }
          />
          <ExplainRow
            emoji="📅"
            title="Cuándo pagar"
            text="Los impuestos del mes se declaran a más tardar el día 17 del mes siguiente. Anota la fecha en tu calendario."
          />
          <ExplainRow
            emoji="📸"
            title="Guarda tus tickets"
            text="Sube fotos de tus facturas y tickets en Mi expediente. Así no se pierden y los tienes a la mano."
          />
        </div>
        ...
        <div className="rounded-xl bg-warning-light p-4 text-sm">
          ...
          <p className="text-muted-foreground mt-1">
            Este es un estimado para que tengas una idea. Para tu declaración oficial, apóyate en un contador o consulta directamente al SAT.
          </p>
        </div>
...
function ExplainRow({ emoji, title, text }: { emoji: string; title: string; text: string }) {
  return (
