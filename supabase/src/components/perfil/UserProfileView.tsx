...
import { Briefcase, FileText, Phone, Store, User, UserPlus, Trash2, TrendingUp, TrendingDown, Camera, HandCoins } from "lucide-react";
...
export function UserProfileView() {
  ...
  async function asignar() {
    if (!user || !emailContador.trim()) { toast.error("Escribe el correo de tu contador"); return; }
    setBusy(true);
    const { data: contadorId, error: lookupErr } = await supabase
      .rpc("buscar_contador_por_correo", { _email: emailContador.trim() });
    if (lookupErr || !contadorId) {
      setBusy(false);
      toast.error("No encontramos un contador con ese correo");
      return;
    }
    const { error } = await supabase.from("contador_clientes").insert({
      contador_id: contadorId, cliente_id: user.id, invitado_por: user.id, estatus: "ACTIVO",
    });
    setBusy(false);
    if (error) { toast.error("No se pudo asignar. ¿Ya lo tienes asignado?"); return; }
  ...
  async function quitar() {
    if (!contador) return;
    setBusy(true);
    const { error } = await supabase.from("contador_clientes").delete().eq("id", contador.id);
    setBusy(false);
